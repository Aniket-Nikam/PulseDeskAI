...
async def enroll_device(payload: DeviceEnrollRequest, db: AsyncSession) -> dict:
    ...
    if existing_device:
        if existing_device.status == DeviceStatus.approved:
            # Do not disclose approved tokens from a public enrollment endpoint.
            return {
                "device_id": str(existing_device.id),
                "device_token": "",
                "status": "pending",
                "message": "Device already approved. Ask your admin for a secure join code.",
            }
        return {
            "device_id": str(existing_device.id),
...
async def delete_device(device_id: uuid.UUID, db: AsyncSession) -> None:
    ...
    if device.status != DeviceStatus.revoked:
        raise HTTPException(status_code=409, detail="Revoke the device before deleting it")

    # Delete dependent records first to satisfy FK constraints.
    await db.execute(delete(Screenshot).where(Screenshot.device_id == device_id))
    await db.execute(delete(AnomalyLog).where(AnomalyLog.device_id == device_id))
    await db.execute(delete(ActivityEvent).where(ActivityEvent.device_id == device_id))
    await db.execute(delete(WorkSession).where(WorkSession.device_id == device_id))
