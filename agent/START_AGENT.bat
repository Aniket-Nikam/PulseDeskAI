...
for /f "usebackq" %%A in (`powershell -NoProfile -Command "$p = Get-CimInstance Win32_Process | Where-Object { $_.Name -like 'python*' -and $_.CommandLine -like '*agent.py*' }; if ($p) { 'yes' } else { 'no' }"`) do set "ALREADY=%%A"
if /I "%ALREADY%"=="yes" (
